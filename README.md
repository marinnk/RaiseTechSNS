...
- ユーザー名・表示名の部分一致によるユーザー検索API、およびユーザー検索画面（S07。検索結果からのプロフィール遷移）
- 投稿への画像添付（最大4枚、jpg/png、1枚5MB以下、Amazon S3連携）の作成・編集API、およびタイムライン・投稿詳細画面での画像選択・プレビュー・表示UI

これで基本設計書に記載の機能は一通り実装済みです。
...
- [監視設計書](docs/monitoring-design.md)  
  構造化ログ・リクエスト相関ID・監視すべき指標・障害対応フロー（バックエンドのみ、将来のDataDog導入方針を含む）
